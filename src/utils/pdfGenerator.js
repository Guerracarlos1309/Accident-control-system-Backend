...
class PdfGenerator {
  ...
  static drawHeader(doc, title) {
    ...
    doc
      ...
      .fontSize(12)
      .fillColor("#005C9E")
      .text("CORPOELEC", 155, 36);
    doc
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .fillColor("#666666")
      .text("CORPORACIÓN ELÉCTRICA NACIONAL", 155, 50);
    doc
      .font("Helvetica")
      .fontSize(7.5)
      .text(
        "GERENCIA DE AMBIENTE, SEGURIDAD E HIGIENE OCUPACIONAL (ASHO)",
        155,
        61,
      );
    ...
    doc
      ...
      .fontSize(10)
      .fillColor("#E30613")
      .text(title.toUpperCase(), 50, 84, { align: "right" });

    // Thin separator line
    doc
      .moveTo(50, 102)
      .lineTo(doc.page.width - 50, 102)
      .lineWidth(1)
      .strokeColor("#E0E0E0")
      .stroke();
  ...
  static drawFooter(doc) {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.text(
        "Documento generado automáticamente por el Sistema de Control de Accidentes y Gestión ASHO.",
        50,
        doc.page.height - 36,
      );
      doc.text(
  ...
  static drawSectionHeader(doc, title) {
    ...
    doc
      ...
      .fontSize(9.5)
      .fillColor("#005C9E")
      .text(title.toUpperCase(), 58, currentY + 1.5);
    doc.y = currentY + 18;
  }
  ...
  static drawDataRow(doc, label, value, x, y, colWidth = 240) {
    ...
    doc
      ...
      .fontSize(8)
      .fillColor("#111111")
      .text(value || "-", x + 105, y, { width: colWidth - 105 });
  }
  ...
  static drawDataRowWithHeight(doc, label, value, x, y, colWidth = 240) {
    ...
    const labelH = doc
      .font("Helvetica-Bold")
      .fontSize(8)
      .heightOfString(label, { width: labelW });
    const valH = doc
      .font("Helvetica")
      .fontSize(8)
      .heightOfString(value || "-", { width: valW });
  ...
  static generatePayrollPdf(employees, columns) {
    return new Promise((resolve, reject) => {
      ...
      doc
        .rect(50, summaryY, doc.page.width - 100, 32)
        .lineWidth(0.5)
        .strokeColor("#CDD6DC")
        .stroke();
      doc
        ...
        .fontSize(8)
        .fillColor("#005C9E")
        .text("RESUMEN DE NÓMINA GENERAL", 65, summaryY + 12);
      doc
        .font("Helvetica")
        .fontSize(8)
        .fillColor("#333333")
        .text(
          `Total Personal Activo: ${employees.length} trabajadores`,
          225,
          summaryY + 12,
        );
      doc
      ...
      employees.forEach((emp) => {
        ...
        if (isSelected("management"))
          cardFields.push({
            label: "Gerencia:",
            value: emp.management ? emp.management.name : "-",
          });
        if (isSelected("jobTitle"))
          cardFields.push({
            label: "Cargo:",
            value: emp.jobTitle ? emp.jobTitle.name : "-",
          });
        if (isSelected("occupation"))
          cardFields.push({
            label: "Ocupación:",
            value: emp.occupation ? emp.occupation.name : "-",
          });
        if (isSelected("phone"))
          cardFields.push({ label: "Teléfono:", value: emp.phone || "-" });
        ...
        if (isSelected("birthDate"))
          cardFields.push({
            label: "Fec. Nac.:",
            value: this.formatDate(emp.birthDate),
          });
        if (isSelected("email"))
          cardFields.push({ label: "Correo:", value: emp.email || "-" });
        if (isSelected("maritalStatus")) {
          ...
          cardFields.push({
            label: "Edo. Civil:",
            value: statuses[String(emp.maritalStatus)] || "-",
          });
        }
        if (isSelected("dominantHand")) {
          ...
          cardFields.push({
            label: "Lateralidad:",
            value: hands[String(emp.dominantHand)] || "-",
          });
        }
        if (isSelected("educationLevel")) {
          ...
          cardFields.push({
            label: "Niv. Educativo:",
            value:
              levels[String(emp.educationLevel).toLowerCase()] ||
              emp.educationLevel ||
              "-",
          });
        }
        if (isSelected("hireDate"))
          cardFields.push({
            label: "Fec. Ingreso:",
            value: this.formatDate(emp.hireDate),
          });
        if (isSelected("officePhone"))
          cardFields.push({
            label: "Tel. Oficina:",
            value: emp.officePhone || "-",
          });
        if (isSelected("birthPlace"))
          cardFields.push({
            label: "Lugar Nac.:",
            value: emp.birthPlace || "-",
            fullWidth: true,
          });
        if (isSelected("homeAddress"))
          cardFields.push({
            label: "Dirección:",
        ...
        let headerRight = "";
        if (isSelected("personalNumber"))
          headerRight += `FICHA: ${emp.personalNumber || "-"}   `;
        ...
        standardFields.forEach((field, fIdx) => {
          ...
          doc
            ...
            .fontSize(7.5)
            .fillColor("#333333")
            .text(String(field.value), x + 70, y, {
              width: 165,
              lineBreak: false,
            });
        });
        ...
        fullWidthFields.forEach((field, fIdx) => {
          ...
          doc
            ...
            .fontSize(7.5)
            .fillColor("#333333")
            .text(String(field.value), 125, y, { width: 410 });
        });
  ...
  static generateAccidentPdf(accident) {
    return new Promise((resolve, reject) => {
      ...
      const facilityStr = accident.facility
        ? `${accident.facility.name} (${accident.facility.location ? accident.facility.location.name : ""})`
        : "-";

      this.drawDataRow(
        doc,
        "Nro. Control:",
        accident.accidentControlNumber || accident.accident_control_number,
        50,
        startY,
      );
      this.drawDataRow(
      ...
      this.drawDataRow(
        doc,
        "Fecha / Hora:",
        `${dateFormatted} - ${accident.accidentTime || ""}`,
        50,
        startY + 28,
      );
      ...
      this.drawDataRow(
        doc,
        "Gerencia:",
        accident.management ? accident.management.name : "-",
        50,
        startY + 56,
        250,
      );
      ...
      const workTypeStr =
        accident.workType === "ordinario"
          ...
          : accident.workType === "eventual"
            ...
            : accident.workType || "-";
      const magnitudeStr = accident.magnitude
      ...
      this.drawDataRow(
        doc,
        "Tipo Incidente:",
        accident.type ? accident.type.name : "-",
        315,
        startY,
        230,
      );
      ...
      this.drawDataRow(
        doc,
        "Estatus:",
        accident.processStatus ? accident.processStatus.name : "-",
        315,
        startY + 42,
        230,
      );
      this.drawDataRow(
        doc,
        "Año / Periodo:",
        accident.period ? String(accident.period.annuality) : "-",
        315,
        startY + 56,
        230,
      );
      ...
      doc
        ...
        .fontSize(8)
        .fillColor("#222222")
        .text(accident.description || "Sin descripción detallada.", 50, doc.y, {
      ...
      if (employees.length === 0) {
        doc
          ...
          .fontSize(8)
          .fillColor("#666666")
          .text("No se registraron lesionados.", 50, doc.y);
        doc.y += 14;
      } else {
        ...
        doc.text("IDENTIFICACIÓN Y NOMBRES", COL.name, tableStartY + 4, {
          width: 140,
        });
        doc.text("GERENCIA / OCUPACIÓN", COL.mgt, tableStartY + 4, {
          width: 115,
        });
        doc.text("TIPO DE LESIÓN", COL.injury, tableStartY + 4, { width: 80 });
        doc.text("NIVEL / CONSECUENCIA", COL.level, tableStartY + 4, {
          width: 80,
        });
        doc.text("DÍAS", COL.days, tableStartY + 4, {
          width: 38,
          align: "center",
        });
        ...
        employees.forEach((ie, idx) => {
          ...
          const name = ie.employee
            ? `${ie.employee.lastName}, ${ie.employee.firstName}`
            : "-";
          doc.text(name.substring(0, 28), COL.name, rowY + 5, { width: 140 });
          doc.font("Helvetica").fontSize(6.5).fillColor("#666666");
          doc.text(
            `CI: ${ie.employee?.idCard || ie.employeePersonalNumber || "N/E"}`,
            COL.name,
            rowY + 14,
            { width: 140 },
          );
          ...
          // Gerencia / Ocupación
          const mgt = ie.employee?.management
            ? ie.employee.management.name
            : "-";
          const occ = ie.employee?.occupation
            ? ie.employee.occupation.name
            : "-";
          ...
          doc.font("Helvetica").fontSize(7.5).fillColor("#333333");
          doc.text(
            ie.injuryType ? ie.injuryType.name.substring(0, 18) : "-",
            COL.injury,
            rowY + 5,
            { width: 80 },
          );
          doc.font("Helvetica").fontSize(6.5).fillColor("#666666");
          doc.text(
            `Pte: ${ie.affectedArea || "N/E"} / Nat: ${ie.injuryNature || "N/E"}`,
            COL.injury,
            rowY + 14,
            { width: 80 },
          );
          ...
          doc.font("Helvetica").fontSize(6.5).fillColor("#666666");
          doc.text(
            `${ie.injuryConsequence || "N/E"}`.substring(0, 14),
            COL.level,
            rowY + 14,
            { width: 80 },
          );
      ...
      const centerName = accident.medicalCenter
        ? accident.medicalCenter.name
        : accident.medicalCenterName || "-";
      const centerAddr = accident.medicalCenter
        ? accident.medicalCenter.address
        : accident.medicalCenterAddress || "-";
      ...
      if (witnesses.length === 0) {
        doc
          ...
          .fontSize(8)
          .fillColor("#666666")
          .text("No se reportaron testigos presenciales.", 50, doc.y);
        doc.y += 14;
      } else {
        const wText = witnesses
          .map(
            (w, i) =>
              `${i + 1}. ${w.name} (C.I: ${w.idCard || "N/E"} - Tel: ${w.phone || "N/E"})`,
          )
          .join("   |   ");
        doc
          .font("Helvetica")
          .fontSize(7.5)
          .fillColor("#333333")
          .text(wText, 50, doc.y, { width: doc.page.width - 100 });
        doc.y += 20;
      }
      ...
      doc
        ...
        .stroke();
      doc
        .moveTo(doc.page.width - 230, sigY)
    ...
    return new Promise((resolve, reject) => {
      ...
      if (inspection.extinguisherInspection) {
        ...
        this.drawSectionHeader(
          doc,
          `Evaluación de Extintores (${extInsp.brand || "General"})`,
        );
        doc
          .font("Helvetica-Bold")
          .fontSize(8)
          .fillColor("#444444")
          .text(
            `Detalles Ubicación:  ${extInsp.locationDetails || "Instalación general"}`,
            50,
            doc.y,
          );
        doc.y += 14;
          ...
          try {
            ...
            parts.forEach((part) => {
              ...
              if (cleanKey === "Code") data.code = cleanValue;
                ...
                data.maintenancePart = cleanValue.toUpperCase();
              else if (cleanKey === "CK") {
        ...
        const getExtinguisherData = (detail, extInsp) => {
          if (
            ...
            const parsed = parseGeneralObservations(
              extInsp.generalObservations,
            );
            return {
              code:
              ...
              maintenancePart:
                detail.observations &&
                detail.observations.startsWith("Mantenimiento: ")
                  ? detail.observations
                      .replace("Mantenimiento: ", "")
                      .toUpperCase()
                  : "",
          ...
          return parseDetailObservations(
            detail ? detail.observations : "",
            detail ? detail.extinguisherNumber : 1,
          );
        };
        ...
        details.forEach((det, idx) => {
          ...
          const cardHeight =
            70 +
            (parsed.maintenancePart ? 10 : 0) +
            (parsed.observationsText ? 12 : 0);
          ...
          doc
            .font("Helvetica-Bold")
            .fontSize(7.5)
            .fillColor(approved ? "green" : "orange");
          doc.text(
            `OPERATIVIDAD: ${det.generalStatus || "OPERATIVO"}`,
            380,
            rowY + 6.5,
          ...
          doc
            ...
            .fontSize(7.5)
            .fillColor("#444444")
            .text("Especificaciones Técnicas", 55, y_body);

          doc
            .font("Helvetica")
            .fontSize(7)
            .fillColor("#666666")
            .text("Área Ubicado:", 55, y_body + 11);
          doc
            .font("Helvetica-Bold")
            .fontSize(7)
            .fillColor("#333333")
            .text(parsed.physicalArea, 125, y_body + 11, { width: 125 });

          doc
            .font("Helvetica")
            .fontSize(7)
            .fillColor("#666666")
            .text("Agente Impulsor:", 55, y_body + 20);
          doc
            .font("Helvetica-Bold")
            .fontSize(7)
            .fillColor("#333333")
            .text(parsed.impulseType, 125, y_body + 20);

          const rechargeDateStr =
            this.formatDate(det.rechargeDate) === "-"
              ? "N/R"
              : this.formatDate(det.rechargeDate);
          doc
            .font("Helvetica")
            .fontSize(7)
            .fillColor("#666666")
            .text("Fecha Recarga:", 55, y_body + 29);
          doc
            .font("Helvetica-Bold")
            .fontSize(7)
            .fillColor("#333333")
            .text(rechargeDateStr, 125, y_body + 29);

          const expDateStr =
            this.formatDate(det.expirationDate) === "-"
              ? "N/R"
              : this.formatDate(det.expirationDate);
          doc
            .font("Helvetica")
            .fontSize(7)
            .fillColor("#666666")
            .text("Vencimiento:", 55, y_body + 38);
          doc
            .font("Helvetica-Bold")
            .fontSize(7)
            .fillColor("#333333")
            .text(expDateStr, 125, y_body + 38);
          ...
          doc
            ...
            .fontSize(7.5)
            .fillColor("#444444")
            .text("Chequeo Visual del Puesto", 265, y_body);
          ...
          const drawCheckItem = (label, val, x_pos, y_pos) => {
            doc
              ...
              .fontSize(7)
              .fillColor("#555555")
              .text(label, x_pos, y_pos);
            doc
              .font("Helvetica-Bold")
              .fontSize(7)
              .fillColor(val ? "green" : "red")
              .text(val ? "BUENO" : "FALLA", x_pos + 115, y_pos, {
                align: "right",
                width: 45,
              });
          };
          ...
          if (parsed.maintenancePart) {
            doc
              ...
              .fillColor("red")
              .text(
                `REPUESTO / PARTE EN FALLA: ${parsed.maintenancePart}`,
                55,
                nextY,
                { width: 195 },
              );
          ...
          if (parsed.observationsText) {
            doc
              ...
              .fillColor("#555555")
              .text(
                `Obs. del Cilindro: ${parsed.observationsText}`,
                55,
                nextY,
                { width: 480 },
              );
        ...
        doc.text("BUENOS", 255, tableStartY + 4, {
          width: 40,
          align: "center",
        });
        doc.text("FALLAS", 300, tableStartY + 4, {
          width: 40,
          align: "center",
        });
        ...
        details.forEach((det, idx) => {
          ...
          let malos =
            det.totalChecked !== undefined
              ? det.totalChecked - det.operative
              : 0;
          let noExisten = 0;
          let commentText = "";
          ...
          doc.font("Helvetica").fontSize(7.5).fillColor("#333333");
          const categoryName = det.category
            ? det.category.name
            : `Categoría ID: ${det.categoryId}`;
          doc.text(categoryName.substring(0, 48), 55, rowY + 5, { width: 195 });

          doc.text(String(buenos), 255, rowY + 5, {
            width: 40,
            align: "center",
          });
          doc.text(String(malos), 300, rowY + 5, {
            width: 40,
            align: "center",
          });
          doc.text(String(total), 345, rowY + 5, {
            width: 40,
            align: "center",
          });
      ...
      doc.text(
        "DELEGADO DE INSTALACIÓN / PREVENCIÓN",
        doc.page.width - 240,
        sigY + 5,
      );
  ...
  static generateAccidentsListPdf(accidents, columns) {
    return new Promise((resolve, reject) => {
      ...
      accidents.forEach((acc) => {
        ...
        if (isSelected("medicalCenterName"))
          cardFields.push({
            label: "Centro Médico:",
            value: acc.medicalCenterName || "-",
          });
        if (isSelected("medicalObservations"))
          cardFields.push({
            label: "Obs. Médicas:",
            value: acc.medicalObservations || "-",
          });
        if (isSelected("globalObservations"))
          cardFields.push({
            label: "Obs. Globales:",
            value: acc.globalObservations || "-",
          });
  ...
  static generateFacilityFullReport(codeRecord, inspections) {
    return new Promise((resolve, reject) => {
      ...
      if (loc && loc.parish && loc.parish.city && loc.parish.city.name)
        locParts.push(loc.parish.city.name);
      const locationName = locParts.length > 0 ? locParts.join(", ") : "-";

      const dateStr = this.formatDate(codeRecord.date);
      const seqStr = codeRecord.sequence
        ? String(codeRecord.sequence).padStart(3, "0")
        : "-";
      ...
      const h1 = this.drawDataRowWithHeight(
        doc,
        "Instalación / Sede:",
        facilityName,
        50,
      ...
      doc.y += h1 + 6;
      const h2 = this.drawDataRowWithHeight(
        doc,
      ...
      const rowAY = doc.y;
      const h3a = this.drawDataRowWithHeight(
        doc,
        "Código:",
        codeRecord.code,
        50,
      ...
      const h3b = this.drawDataRowWithHeight(
        doc,
        "Tipo de Código:",
        typeLabel,
        315,
      ...
      const rowBY = doc.y;
      const h4a = this.drawDataRowWithHeight(
        doc,
        "Fecha del Informe:",
        dateStr,
        50,
      ...
      const h4b = this.drawDataRowWithHeight(
        doc,
        "Secuencia / Año:",
        `${seqStr} / ${yearStr}`,
        315,
      ...
      const h5a = this.drawDataRowWithHeight(
        doc,
      ...
      const h5b = this.drawDataRowWithHeight(
        doc,
        "Memo de Entrega:",
        memoNumberStr,
        315,
      ...
      if (notesText) {
        ...
        doc
          ...
          .fillColor("#888888")
          .text(
            "Sin resumen o informe registrado para este código.",
            50,
            doc.y,
            {
              width: pageW - 100,
      ...
      this.drawSectionHeader(
        doc,
        `Inspecciones Registradas en la Sede (${inspections.length})`,
      );
      ...
      if (inspections.length === 0) {
        doc
          ...
          .fillColor("#888888")
          .text(
            "No se encontraron inspecciones registradas para esta sede.",
            50,
            doc.y,
            {
              width: pageW - 100,
        ...
        inspections.forEach((insp) => {
          ...
          doc.text(String(codeVal).toUpperCase(), 55, rowY + 6);
          doc.font("Helvetica-Bold").fontSize(8).fillColor("#555555");
          doc.text(`FECHA: ${dateVal}`, 300, rowY + 6, {
            width: 240,
            align: "right",
          });
          doc
      ...
      doc
        ...
        .stroke();
      doc
        .moveTo(pageW - 240, sigY)
        .lineTo(pageW - 80, sigY)
        .stroke();

      doc.font("Helvetica-Bold").fontSize(7).fillColor("#444444");
      doc.text("INSPECTOR / RESPONSABLE ASHO", 80, sigY + 5, {
        width: 160,
        align: "center",
      });
      doc.text("DELEGADO DE INSTALACIÓN / PREVENCIÓN", pageW - 240, sigY + 5, {
        width: 160,
        align: "center",
      });
