...
async function seed() {
  try {
    ...
    const l1 = await Location.create({
      id: 1,
      name: "Sede Principal",
      parishId: p1.id,
    });
    ...
    const vModel = await Model.create({
      id: 1,
      name: "Hilux",
      brandId: brand.id,
    });
    ...
    const pec1 = await ProtectionEquipmentCategory.create({
      id: 1,
      name: "Cabeza",
    ...
    const acc = await Accident.create({
      ...
      accidentCode: "ACC-2026-001",
      description:
        "Evento técnico en pórtico de entrada por falla de aislamiento.",
      facilityId: fac1.id,
      accidentTypeId: 3,
      periodId: 1,
      userId: 1,
    ...
    await EmployeeAccident.create({
      accidentId: acc.id,
      employeePersonalNumber: "EMP001",
      injuryTypeId: 1,
      magnitudeId: 4,
      restDays: 15,
    });
    await AccidentDocumentCheck.create({
      accidentId: acc.id,
      documentId: 1,
      delivered: true,
    ...
    await ExtinguisherDetail.create({
      extinguisherInspectionId: 1,
      agentTypeId: 1,
      capacity: "10lb",
      status: "Operativo",
    });
    ...
    const protInsp = await ProtectionInspection.create({
      id: 1,
      inspectionId: insp.id,
      responsiblePersonalNumber: "EMP001",
    });
    await ProtectionInspectionDetails.create({
      protectionInspectionId: 1,
      categoryId: 1,
      stock: 45,
      required: 5,
    });
